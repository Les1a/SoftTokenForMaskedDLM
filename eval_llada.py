    ...
    def generate_until(self, requests):
        ...
        for batch in tqdm(batched_requests, desc="Generating..."):
            ...
            if self.dual_branch:
                if self.use_cache:
                    if self.dual_cache:
                        generated_answer, nfe = generate_with_dual_cache_dual_branch(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, block_length=self.block_length, 
                                            temperature=0, remasking=self.remasking, mask_id=self.mask_id, factor=self.factor,
                        ...
                        generated_answer, nfe = generate_with_prefix_cache_dual_branch(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, block_length=self.block_length, 
                                            temperature=0, remasking=self.remasking, mask_id=self.mask_id, factor=self.factor,
                                            main_threshold=self.main_threshold, spec_threshold=self.spec_threshold, merge_window=self.merge_window, evolution_interval=self.evolution_interval)
                else:
                    generated_answer, nfe = generate_dual_branch(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, block_length=self.block_length, 
                                            temperature=0, remasking=self.remasking, mask_id=self.mask_id, factor=self.factor,
                                            main_threshold=self.main_threshold, spec_threshold=self.spec_threshold, merge_window=self.merge_window, evolution_interval=self.evolution_interval)
            else:
                if self.use_cache:
